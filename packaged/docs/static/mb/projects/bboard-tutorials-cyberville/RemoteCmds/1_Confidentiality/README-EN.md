...
__1.__ Load __Mission Fake Parking ID__ into 📳 M5 module. You can see this [__video__](https://www.canva.com/design/DAGJhm69_Mk/JdN1bb74mN-bKiclzST5Ag/watch?utm_content=DAGJhm69_Mk&utm_campaign=designshare&utm_medium=link&utm_source=editor) if you want to remember how to do it.
...
<br>
__4.__ Once the listing table is done. they must send a CONFIDENTIAL (private) message to each 📮 IP on the network using the 🧩 block ``||MSG to specific IP||`` asking for a partner with the same Autentication Code.

__5.__ Students with the same code, for example: __QSX1__. They will send a message back to doble ckeck with the Two Factor Authentication code, sending a __CONFIDENTIAL__ mesage with their MFA, for example: ZS3E.
...
input.onButtonPressed(Button.A, function () {
 Cybersec.MSG_UPD_SndtoAll(Cybersec.blixel_indexR(BLiXelIndexR.one), "Hello")
})
input.onButtonPressed(Button.B, function () {
 Cybersec.TXT_UPD_Snd("Hi my authentication code is QSX1, what is yours?", "192.168.4.1")
})
Cybersec.WifiConnect("Cyberville #1", "BL_Cybr1")
Cybersec.Rdy2listen()
